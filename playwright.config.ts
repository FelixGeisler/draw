...
// API_PORT/VITE_PORT can be overridden so parallel checkouts (e.g. git
// worktrees) can run the suite side by side without port collisions.
const E2E_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "draw-e2e-"));
const API_PORT = process.env.API_PORT || "3101";
const VITE_PORT = process.env.VITE_PORT || "5273";
