...
// The colocated spec (paletteNav.test.ts) is the contract; these bodies are
// TEST-FIRST SKELETONS that land with the implementation.
...
export function moveSelection(count: number, index: number, delta: -1 | 1): number {
  ...
  void index;
  void delta;
  throw new Error("TODO #243: implement moveSelection");
}
...
export function commandForEntry(entry: PaletteEntry, withCtrl: boolean): PaletteCommand {
  void entry;
  void withCtrl;
  throw new Error("TODO #243: implement commandForEntry");
}
...
export function timerToggleCommand(
  ...
  void runningTaskId;
  void currentDrawTaskId;
  throw new Error("TODO #243: implement timerToggleCommand");
}
