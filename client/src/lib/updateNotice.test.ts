...
describe("serializeUpdateNotice / parseUpdateNotice", () => {
  it("round-trips the version the toast will announce", () => {
    expect(parseUpdateNotice(serializeUpdateNotice("1.2.0"))).toBe("1.2.0");
  });

  it("nothing stored means nothing to announce", () => {
  ...
  it("a corrupt flag must not toast gibberish", () => {
...
describe("shouldReloadAfterApply", () => {
  it("reloads only when the server reports a DIFFERENT version", () => {
    expect(shouldReloadAfterApply("1.0.0", "1.1.0")).toBe(true);
  });

  it("the same version is never a reason to reload — no reload loops", () => {
    expect(shouldReloadAfterApply("1.0.0", "1.0.0")).toBe(false);
  });

  it("a failed poll tick (container mid-restart) never reloads", () => {
