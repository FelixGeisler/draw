...
└── services/                Domain logic
    ...
    ├── svgSanitizer.ts      allowlist re-serializer for model-written SVG (ADR-21): pure
...
|Card art safety and cost (ADR-21)
...
`tasks`/`gamification`/`stats`/`goals` query keys (goal cards derive their
task counts from tasks). There is no other state management.
