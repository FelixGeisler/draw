...
`npm install` still reports the known `inflight`/`glob` deprecations described
below. Other install warnings and audit findings are not assessed or waived
here; they remain outside issue #320 and must follow the normal issue and review
workflow.
...
The former Antora `js-yaml@4.1.1` advisory note was retired in issue #320:
Antora 3.2.0 resolves `js-yaml@5.4.2`, outside the affected 4.x range for
[GHSA-h67p-54hq-rp68](https://github.com/advisories/GHSA-h67p-54hq-rp68).
This records that the revisit trigger was acted on; it does not claim that all
dependency alerts were eliminated.

**Decision:** continue accepting only the docs-toolchain warning above; make no
further dependency changes in issue #320. Revisit when either of these holds:

- `asciidoctor-kroki` / `@asciidoctor/core` ship a release that drops the old
  `glob` chain → drop the `inflight`/`glob` note.
- The remaining warning's severity is raised, or either package enters the server
...
The PR begins in `PRE_ACTIVATION_EVIDENCE_PENDING`: it must not close #286 or
merge while criterion 5's preactivation evidence is incomplete. Only after all
of those gates pass may the coordinator transition it to
`MERGE_PENDING_PRODUCTION_ACCEPTANCE`. Merge still does not prove unattended
replacement; #286 stays open until the secret-free production evidence packet
receives the human's exact `ACCEPT <packet-sha256>` statement. Never put a
password, token, Docker auth content, rendered secret configuration, or
sensitive backup path in a commit, PR, issue, log, or evidence packet.
